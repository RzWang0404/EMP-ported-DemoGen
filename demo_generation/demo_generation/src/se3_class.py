...
class se3_class:
    def __init__(self, output_path, p_in:np.ndarray, q_in:list, p_out:np.ndarray, q_out:list, p_att:np.ndarray, q_att:R, dt:float, K_init:int, task_id:int) -> None:
        """
        Parameters:
        ----------
        ...
        self.output_path    = output_path#os.path.dirname(output_path)
    ...
    def _logOut(self, write_json=True):
        ds_linear  = self.pos_ds._logOut(write_json, self.output_path)
        ds_angular = self.ori_ds._logOut(write_json, self.output_path)
        
        if write_json:
            os.rename(self.output_path + "/0.json", self.output_path + "/" + str(self.task_id)+"_"+"pos.json")
            os.rename(self.output_path + "/1.json", self.output_path + "/" + str(self.task_id)+"_"+"ori.json")
        
        return (ds_linear, ds_angular) # tuple


    def begin(self, write_json=True):
        self._cluster()
        self._optimize()
        return self._logOut(write_json)
    ...
    def sim(self, p_init, q_init, step_size):
        ...
        while np.linalg.norm((q_test[-1] * self.q_att.inv()).as_rotvec()) >= self.tol or np.linalg.norm((p_test[-1] - self.p_att)) >= self.tol:
            if i > self.max_iter:
                ...
                print("Distance to Pos att: ", np.linalg.norm((p_test[-1] - self.p_att)))
                print("Distance to Ori att: ", np.linalg.norm((q_test[-1] * self.q_att.inv()).as_rotvec()))
