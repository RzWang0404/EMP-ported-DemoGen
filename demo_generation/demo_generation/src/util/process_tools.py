...
def _flip_quat(q_raw):
    """flip the opposite quaternions"""
    L = len(q_raw)
    q_new = []
    for l in range(L):

        q_arr = list_to_arr(q_raw[l])

        q_old = q_arr[0, :]
        for i in np.arange(1, q_arr.shape[0]):
            if np.linalg.norm(q_arr[i, :] - q_old) > 1:
                q_arr[i, :] *= -1
            q_old = q_arr[i, :]
        q_new.append([R.from_quat(q_arr[i, :]) for i in range(q_arr.shape[0])])
